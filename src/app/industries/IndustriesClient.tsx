...
const fallbackPageData = {
  heroSubtitle: 'INDUSTRIES',
  heroTitle: 'Digital Excellence Across Industries',
  heroHighlight: 'Industries',
  heroDescription: 'We understand the unique challenges of different sectors and deliver tailored solutions that drive results.',
  ctaTitle: "Don't See Your Industry?",
  ctaDescription: 'We work with businesses across all sectors. Contact us to discuss how we can create a custom solution for your industry.',
  ctaButtonText: 'Contact Us',
  ctaSecondaryText: 'Get Free Audit'
};
...
export default function IndustriesClient({ pageData, industries }: IndustriesClientProps) {
    ...
    <div className="overflow-hidden">
      ...
      <section className="relative py-32" style={{ backgroundImage: 'url(/hero2.jpg)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            <span className="inline-block text-[#F15924] font-semibold mb-4">{page.heroSubtitle || 'INDUSTRIES'}</span>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6">
              {page.heroTitle?.replace(page.heroHighlight || '', '') || 'Digital Excellence Across '}{' '}
              <span className="text-gradient-orange">{page.heroHighlight || 'Industries'}</span>
            </h1>
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">
              {page.heroDescription || 'We understand the unique challenges of different sectors and deliver tailored solutions that drive results.'}
            </p>
          </motion.div>
        </div>
      </section>
          ...
          >
            {industriesList.map((industry, index) => {
              ...
              return (
                <motion.a
                  key={index}
                  href={`#${industry.slug || industry.name.toLowerCase().replace(' ', '-')}`}
                  variants={fadeInUp}
                ...
                >
                  <div className="w-12 h-12 rounded-lg bg-[#F15924]/20 group-hover:bg-[#F15924] mx-auto flex items-center justify-center mb-3 transition-colors">
                    <IconComponent size={24} className="text-[#F15924] group-hover:text-white transition-colors" />
                  </div>
      ...
      {industriesList.map((industry, index) => {
        ...
        <section 
          key={industry.name}
          id={industry.slug || industry.name.toLowerCase().replace(' ', '-')}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">
              {page.ctaTitle || "Don't See Your Industry?"}
            </h2>
            <p className="text-xl text-gray-300 mb-10 max-w-2xl mx-auto">
              {page.ctaDescription || 'We work with businesses across all sectors. Contact us to discuss how we can create a custom solution for your industry.'}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/contact" className="btn-primary flex items-center justify-center gap-2">
                {page.ctaButtonText || 'Contact Us'} <ArrowRight size={18} />
              </Link>
              <Link href="/free-audit" className="btn-secondary flex items-center justify-center gap-2">
                {page.ctaSecondaryText || 'Get Free Audit'}
              </Link>
            </div>
          </motion.div>
        </div>
